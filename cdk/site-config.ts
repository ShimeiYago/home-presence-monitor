...
export function loadSiteConfig(configPath: string = DEFAULT_CONFIG_PATH): SiteConfig {
  ...
  if (parsed.domainName && !parsed.hostedZoneDomain) {
    throw new Error('site-config.json must specify "hostedZoneDomain" when "domainName" is set.');
  }

  if (!parsed.domainName && parsed.hostedZoneDomain) {
    throw new Error('site-config.json must specify "domainName" when "hostedZoneDomain" is set.');
  }
